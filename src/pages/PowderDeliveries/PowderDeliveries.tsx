...
const Field = ({ label, value }: { label: string; value: React.ReactNode }) => <div className={styles.field}><span>{label}</span><strong>{value ?? "-"}</strong></div>;

const useActiveAgents = () => {
  const [agents, setAgents] = useState<AgentDto[]>([]);
  useEffect(() => { void agentsService.getAgents({ page: 1, pageSize: 200, status: 0 }).then((response) => setAgents(response.results ?? [])).catch((error) => toast.error(getApiErrorMessage(error, "Failed to load agents."))); }, []);
  return agents;
};
...
const useDeliveryColumns = (confirmingId: string | null = null, onConfirm?: (id: string) => Promise<void>) => {
  ...
  return useMemo(() => [
    ...
    { id: "date", header: "Delivery Date", cell: ({ row }: any) => timestamp(row.original.deliveryDate) },
    { id: "agent", header: "Agent", cell: ({ row }: any) => `${row.original.agent.code} - ${row.original.agent.fullName}` },
    { id: "contract", header: "Contract", cell: ({ row }: any) => row.original.contract.contractNumber },
...
export const CreatePowderDelivery = () => {
  const navigate = useNavigate(); const agents = useActiveAgents(); const [contracts, setContracts] = useState<AgentContractListItemDto[]>([]); const [advances, setAdvances] = useState<AgentAdvanceDto[]>([]); const [agentId, setAgentId] = useState(""); const [contractId, setContractId] = useState(""); const [advanceId, setAdvanceId] = useState(""); const [deliveryDate, setDeliveryDate] = useState(new Date().toISOString().slice(0, 16)); const [gross, setGross] = useState(""); const [net, setNet] = useState(""); const [pt, setPt] = useState(""); const [pd, setPd] = useState(""); const [rh, setRh] = useState(""); const [notes, setNotes] = useState(""); const [saving, setSaving] = useState(false); const [errors, setErrors] = useState<Record<string, string>>({});
  useEffect(() => { setContractId(""); setAdvanceId(""); setAdvances([]); if (!agentId) { setContracts([]); return; } void agentContractsService.listContracts({ agentId, status: "Active", page: 1, pageSize: 200 }).then((response) => setContracts(response.results ?? [])).catch((error) => toast.error(getApiErrorMessage(error, "Failed to load active contracts."))); }, [agentId]);
  useEffect(() => { setAdvanceId(""); if (!contractId) { setAdvances([]); return; } void agentContractsService.listContractAdvances(contractId).then((response) => setAdvances(response.filter((item) => active(item.status)))).catch((error) => toast.error(getApiErrorMessage(error, "Failed to load active advances."))); }, [contractId]);
  const submit = async () => { const grossValue = Number(gross); const netValue = Number(net); const ptValue = Number(pt || 0); const pdValue = Number(pd || 0); const rhValue = Number(rh || 0); const next: Record<string, string> = {}; if (!agentId) next.agent = "Agent is required."; if (!contractId) next.contract = "Active contract is required."; if (!deliveryDate) next.date = "Delivery date is required."; if (!(grossValue > 0)) next.gross = "Gross weight must be greater than 0."; if (!(netValue > 0)) next.net = "Net weight must be greater than 0."; else if (netValue > grossValue) next.net = "Net weight cannot exceed gross weight."; if ([ptValue, pdValue, rhValue].some((value) => value < 0) || !(ptValue > 0 || pdValue > 0 || rhValue > 0)) next.metals = "Enter a non-negative metal amount and at least one value greater than 0."; setErrors(next); if (Object.keys(next).length) return; setSaving(true); try { const id = await powderDeliveriesService.create({ agentId, agentContractId: contractId, agentAdvanceId: advanceId || null, deliveryDate: new Date(deliveryDate).toISOString(), grossWeightKg: grossValue, netWeightKg: netValue, ptGrams: ptValue, pdGrams: pdValue, rhGrams: rhValue, notes: notes.trim() || null }); toast.success("Powder delivery created."); navigate(`/powder-deliveries/${id}`); } catch (error) { toast.error(getApiErrorMessage(error, "Failed to create powder delivery.")); } finally { setSaving(false); } };
  return <div className={styles.page}><SectionHeader title="Create Powder Delivery" goBack /><div className={styles.form}><div><label>Agent</label><Select value={agentId} onChange={(event) => setAgentId(event.target.value)}><option value="">Select active agent</option>{agents.map((agent) => <option key={agent.id} value={agent.id}>{agentLabel(agent)}</option>)}</Select>{errors.agent && <small className={styles.error}>{errors.agent}</small>}</div><div><label>Active Contract</label><Select value={contractId} disabled={!agentId} onChange={(event) => setContractId(event.target.value)}><option value="">Select active contract</option>{contracts.map((contract) => <option key={contract.id} value={contract.id}>{contract.contractNumber}</option>)}</Select>{errors.contract && <small className={styles.error}>{errors.contract}</small>}</div><div><label>Active Advance (optional)</label><Select value={advanceId} disabled={!contractId} onChange={(event) => setAdvanceId(event.target.value)}><option value="">No advance</option>{advances.map((advance) => <option key={advance.id} value={advance.id}>{advance.advanceNumber}</option>)}</Select></div><TextField label="Delivery Date" type="datetime-local" value={deliveryDate} onChange={(event) => setDeliveryDate(event.target.value)} error={Boolean(errors.date)} helperText={errors.date} /><div className={styles.grid}><TextField label="Gross Weight (kg)" type="number" min="0.001" value={gross} onChange={(event) => setGross(event.target.value)} error={Boolean(errors.gross)} helperText={errors.gross} /><TextField label="Net Weight (kg)" type="number" min="0.001" value={net} onChange={(event) => setNet(event.target.value)} error={Boolean(errors.net)} helperText={errors.net} /></div><div className={styles.grid}><TextField label="Pt (grams)" type="number" min="0" value={pt} onChange={(event) => setPt(event.target.value)} /><TextField label="Pd (grams)" type="number" min="0" value={pd} onChange={(event) => setPd(event.target.value)} /><TextField label="Rh (grams)" type="number" min="0" value={rh} onChange={(event) => setRh(event.target.value)} /></div>{errors.metals && <small className={styles.error}>{errors.metals}</small>}<Textarea label="Notes" value={notes} onChange={(event) => setNotes(event.target.value)} /><div className={styles.actions}><Button variant="secondary" onClick={() => navigate(-1)}>Cancel</Button><Button onClick={submit} disabled={saving}>{saving ? "Creating..." : "Create Delivery"}</Button></div></div></div>;
};

export const PowderDeliveryDetails = () => {
  const { id } = useParams(); const navigate = useNavigate(); const [delivery, setDelivery] = useState<PowderDeliveryDto | null>(null); const [loading, setLoading] = useState(true); const [action, setAction] = useState<"confirm" | "cancel" | null>(null); const [working, setWorking] = useState(false);
  const load = async () => { if (!id) return; setLoading(true); try { setDelivery(await powderDeliveriesService.get(id)); } catch (error) { toast.error(getApiErrorMessage(error, "Failed to load powder delivery.")); } finally { setLoading(false); } };
...
export const PowderDeliveryHistory = ({ kind }: { kind: "agent" | "contract" }) => {
  const { id } = useParams(); const [searchParams, setSearchParams] = useSearchParams(); const columns = useDeliveryColumns(); const [results, setResults] = useState<PowderDeliveryListItemDto[]>([]); const [total, setTotal] = useState(0); const [loading, setLoading] = useState(false); const page = Number(searchParams.get("page") || 1); const pageSize = Number(searchParams.get("pageSize") || 10); const status = searchParams.get("status") || ""; const number = searchParams.get("deliveryNumber") || ""; const from = searchParams.get("deliveryDateFrom") || ""; const to = searchParams.get("deliveryDateTo") || "";
  const filter = (key: string, value: string) => { const next = new URLSearchParams(searchParams); value ? next.set(key, value) : next.delete(key); next.set("page", "1"); setSearchParams(next); };
  useEffect(() => { if (!id) return; const load = async () => { setLoading(true); try { const params = { page, pageSize, status: status || undefined, deliveryNumber: number || undefined, deliveryDateFrom: from || undefined, deliveryDateTo: to || undefined }; const response = kind === "agent" ? await powderDeliveriesService.listForAgent(id, params) : await powderDeliveriesService.listForContract(id, params); setResults(response.results ?? []); setTotal(response.totalItems); } catch (error) { toast.error(getApiErrorMessage(error, "Failed to load delivery history.")); } finally { setLoading(false); } }; void load(); }, [id, kind, page, pageSize, status, number, from, to]);
  return <div className={styles.page}><SectionHeader title={kind === "agent" ? "Agent Delivery History" : "Contract Delivery History"} goBack /><div className={styles.filters}><Select value={status} onChange={(event) => filter("status", event.target.value)}><option value="">All statuses</option>{statuses.map((item) => <option key={item} value={item}>{item}</option>)}</Select><TextField label="Delivery Number" value={number} onChange={(event) => filter("deliveryNumber", event.target.value)} /><TextField label="From" type="date" value={from} onChange={(event) => filter("deliveryDateFrom", event.target.value)} /><TextField label="To" type="date" value={to} onChange={(event) => filter("deliveryDateTo", event.target.value)} /></div><div className={styles.summary}><span>{total} deliveries</span><Button variant="secondary" size="small" onClick={() => setSearchParams({ page: "1", pageSize: String(pageSize) })}>Reset</Button></div><DataTable columns={columns as any} data={results} isLoading={loading} manualPagination pageCount={Math.max(1, Math.ceil(total / pageSize))} pageIndex={page - 1} onPaginationChange={(index) => filter("page", String(index + 1))} noResultsText="No powder deliveries found" loadingText="Loading powder deliveries..." /></div>;
};